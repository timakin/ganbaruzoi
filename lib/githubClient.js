...
class GithubClient {
    ...
    pushDailyReport(reportName, content, cb) {
        ...
        ghrepo.createContents('daily/#{recordedMonth}/#{reportName}', reportName + ' is uploaded.', content, cb);
    }
        ...
        ghrepo.contents('current.json', (err, body) => {
            if (err) {
                if (err.message == "Not Found") {
                    ghrepo.createContents('current.json', 'A current status recorder is created', currentJson, (err, body) => {
                        if (err) {
                            console.error(err);
                        } else {
                            return cb(body.content);
                        }
                    });
                } else {
                    console.error(err);
    ...
    updateCurrentStatus(content, cb) {
        ghrepo.contents('current.json', (err, body) => {
            if (err) {
                ...
                ghrepo.updateContents('current.json', 'Status is updated', content, body.sha, (err, body) => {
                    if (err) {
                        console.error(err);
                    } else {
                        cb(body.content);
